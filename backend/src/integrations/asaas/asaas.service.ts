...
export class AsaasService {
    ...
    constructor(private configService: ConfigService) {
        ...
        if (!this.apiKey) {
            this.logger.warn('ASAAS_API_KEY is not set. Asaas integration will not work.');
        }
    }
    ...
    private async request(endpoint: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE', body?: any) {
        ...
        try {
            ...
            if (!response.ok) {
                this.logger.error(`Asaas API Error [${method} ${url}]: ${JSON.stringify(data)}`);
                const errorMessage = data.errors?.[0]?.description || 'Error communicating with Asaas';
    ...
    async createSubscription(data: {
        ...
        if (data.billingType === 'CREDIT_CARD') {
            if (!data.creditCard || !data.creditCardHolderInfo) {
                throw new Error('Credit card details and holder info are required for CREDIT_CARD billing type');
            }
        }
    ...
    async createPayment(data: { customer: string; billingType: string; value: number; dueDate: string; description?: string }) {
